...
// DB is set by the db package after initialization.
var DB *sql.DB
...
// Entry represents a food consumption log entry.
type Entry struct {
	ID        int64     `json:"id"`
...
// MealTemplate is a saved meal configuration.
...
// PlannedMeal is a scheduled meal from a template.
...
// DaySummary contains aggregated nutritional data for a day.
...
// SearchItem represents a unified search result for foods and ingredients.
type SearchItem struct {
	Type        string  `json:"type"` // "food" or "ingredient"
	ID          int64   `json:"id"`
	Name        string  `json:"name"`
	Calories    int     `json:"calories"`
	Protein     float64 `json:"protein"`
